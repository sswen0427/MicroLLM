...
TEST(GenerationAlignmentTest, TinyLlamaGreedyGenerationMatchesHfReference) {
  ...
  const std::filesystem::path model_dir = root_path /
                                          "data/my_tinyllama/AI-ModelScope" /
  ...
  EXPECT_TRUE(model_or.ok()) << model_or.status();
  ...
  EXPECT_TRUE(tokenizer_or.ok()) << tokenizer_or.status();
  ...
  EXPECT_TRUE(result_or.ok()) << result_or.status();
