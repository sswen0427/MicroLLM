...
namespace op {
Layer::Layer(LayerType layer_type, base::DataType data_type,
...
ParamLayerFp32::ParamLayerFp32(LayerType layer_type, std::string layer_name)
    : Layer(layer_type, base::DataType::kDataTypeFp32, std::move(layer_name)) {}
...
void ParamLayerFp32::set_input(int32_t idx, const tensor::Tensor &input) {
  CHECK_GE(idx, 0);
  CHECK_LT(idx, input.size());
  CHECK(input.data_type() == base::DataType::kDataTypeFp32);
  inputs_.at(idx) = input;
}

void ParamLayerFp32::set_output(int32_t idx, const tensor::Tensor &output) {
  CHECK_GE(idx, 0);
  CHECK_LT(idx, output.size());
  CHECK(output.data_type() == base::DataType::kDataTypeFp32);
  outputs_.at(idx) = output;
}

tensor::Tensor ParamLayerFp32::get_input(int32_t idx) const {
  CHECK_GE(idx, 0);
  CHECK_LT(idx, inputs_.size());
  return inputs_.at(idx);
}

tensor::Tensor ParamLayerFp32::get_output(int32_t idx) const {
  CHECK_GE(idx, 0);
  CHECK_LT(idx, outputs_.size());
  return outputs_.at(idx);
}

void ParamLayerFp32::set_weight(int32_t idx, const tensor::Tensor &weight) {
  CHECK_GE(idx, 0);
  CHECK_LT(idx, weight.size());
  CHECK(weight.data_type() == base::DataType::kDataTypeFp32);
  weights_.at(idx) = weight;
}

tensor::Tensor ParamLayerFp32::get_weight(int32_t idx) const {
  CHECK_GE(idx, 0);
