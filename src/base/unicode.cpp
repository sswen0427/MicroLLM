...
static std::vector<size_t> unicode_regex_split_stl(
    const std::wstring &wtext, const std::wstring &regex_expr,
  ...
  size_t start = 0;
  for (auto offset : offsets) {
    std::wcregex_iterator it(wtext.data() + start,
                             wtext.data() + start + offset, expr);
...
static std::vector<size_t> unicode_regex_split_stl(
    const std::string &text, const std::string &regex_expr,
    const std::vector<size_t> &offsets) {
  ...
  size_t start = 0;
  for (auto offset : offsets) {
    std::cregex_iterator it(text.data() + start, text.data() + start + offset,
                            expr);
...
std::vector<uint32_t> unicode_cpts_normalize_nfd(
  ...
  for (size_t i = 0; i < cpts.size(); ++i) {
    ...
    auto it = std::upper_bound(unicode_ranges_nfd.cbegin(),
                               unicode_ranges_nfd.cend(), cpt, comp) -
              1;
  ...
  if (need_collapse) {
    ...
    for (size_t i = 0; i < cpts.size(); ++i) {
      ...
      if (flags.is_whitespace) {
        ...
        // fallback
        text_collapsed[i] =
            (char)0x0B;  // <vertical tab> as whitespace fallback
