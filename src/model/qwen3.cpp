...
void Qwen3Layers::to_cuda(std::shared_ptr<base::CudaConfig> config) {
  if (add_layer_) {
...
Qwen3Model::Qwen3Model(base::TokenizerType tokenizer_type,
...
void Qwen3Model::create_param_layers() {
  ...
  qwen_layers_->embedding_layer_ = std::make_shared<op::EmbeddingLayer>(
      device_type_, hidden_dim, config_->seq_len_,
  ...
  for (int32_t i = 0; i < config_->layer_num_; ++i) {
    auto wq =
        std::make_shared<op::MatmulLayer>(device_type_, dim, hidden_dim, false);
  ...
  for (int32_t i = 0; i < config_->layer_num_; ++i) {
    ...
    rms_norm_layer->set_weight(0, {config_->head_size_},
  ...
  // key
  for (int32_t i = 0; i < config_->layer_num_; ++i) {
    auto wk = std::make_shared<op::MatmulLayer>(device_type_, hidden_dim,
                                                kv_dim, false);
  ...
  // value
  for (int32_t i = 0; i < config_->layer_num_; ++i) {
    auto wv = std::make_shared<op::MatmulLayer>(device_type_, hidden_dim,
                                                kv_dim, false);
  ...
  for (int32_t i = 0; i < config_->layer_num_; ++i) {
    auto wo =
        std::make_shared<op::MatmulLayer>(device_type_, hidden_dim, dim, false);
  ...
  for (int32_t i = 0; i < config_->layer_num_; ++i) {
    auto w1 = std::make_shared<op::MatmulLayer>(device_type_, immediate_dim,
                                                hidden_dim, false);
    w1->set_weight(0, {immediate_dim, hidden_dim},
  ...
  // w2 layers
  for (int32_t i = 0; i < config_->layer_num_; ++i) {
    auto w2 = std::make_shared<op::MatmulLayer>(device_type_, hidden_dim,
                                                immediate_dim, false);
    w2->set_weight(0, {hidden_dim, immediate_dim},
                   this->raw_model_data_->weight(pos), cpu_device_type);
  ...
  // w3 layers
  for (int32_t i = 0; i < config_->layer_num_; ++i) {
    auto w3 = std::make_shared<op::MatmulLayer>(device_type_, immediate_dim,
                                                hidden_dim, false);
    w3->set_weight(0, {immediate_dim, hidden_dim},
                   this->raw_model_data_->weight(pos), cpu_device_type);
    qwen_layers_->w3_layers_.push_back(w3);
    pos = pos + immediate_dim * hidden_dim;
  }

  auto lm_head = std::make_shared<op::MatmulLayer>(
      device_type_, config_->vocab_size_, config_->hidden_dim_, false);
...
void Qwen3Model::init_mem() {
  ...
  tensor::Tensor rms_output(base::DataType::kDataTypeFp32, config_->hidden_dim_,
                            true, alloc);
  tensor::Tensor out_mha(base::DataType::kDataTypeFp32, config_->dim_, true,
                         alloc);
  ...
  tensor::Tensor w1_output(base::DataType::kDataTypeFp32,
                           config_->immediate_dim_, true, alloc);
  ...
  tensor::Tensor attn_output(base::DataType::kDataTypeFp32,
                             config_->hidden_dim_, true, alloc);
...
void Qwen3Model::attention_qkv(int32_t layer_idx,
  ...
  query.reshape(
      {(int32_t)query.size() / config_->head_size_, config_->head_size_});
...
void Qwen3Model::feed_forward(int32_t layer_idx,
                              const tensor::Tensor& input) const {
  CHECK(qwen_layers_ != nullptr);
  // residual add
...
op::EmbeddingOutput Qwen3Model::embedding(
  ...
  if (input_tokens.size() != tokens.size()) {
    ...
    input_embeddings.reshape(
        {static_cast<int32_t>(tokens.size()), config_->hidden_dim_});
  }
...
int32_t Qwen3Model::post_processing(const tensor::Tensor& pos,
