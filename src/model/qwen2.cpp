...
absl::Status Qwen2Model::init(base::DeviceType device_type) {
...
absl::Status Qwen2Model::forward(const tensor::Tensor& input,
...
void Qwen2Model::create_param_layers() {
  ...
  for (int32_t i = 0; i < config_->layer_num_; ++i) {
    auto wq =
        std::make_shared<op::MatmulLayer>(device_type_, dim, dim, false, true);
    wq->set_weight(0, {dim, dim}, this->raw_model_data_->weight(pos),
                   cpu_device_type);
    pos += dim * dim;
...
absl::Status Qwen2Model::create_layers() {
  using namespace base;
  if (!qwen_layers_) {
...
absl::Status Qwen2Model::predict(const tensor::Tensor& input,
                                 const tensor::Tensor& pos_tensor,
