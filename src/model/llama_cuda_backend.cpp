...
void EnsureCudaLayerCache(LlamaLayerCache &cache,
                          const HfLlamaConfig &config,
                          const LlamaForwardState &state) {
  if (!cache.cuda_key.is_empty() && !cache.cuda_value.is_empty()) {
    return;
  }
  cache.cuda_key = tensor::Tensor::allocate(
      base::DataType::kDataTypeFp32,
      {config.max_position_embeddings, state.kv_dim},
      base::DeviceType::kDeviceCUDA);
  cache.cuda_value = tensor::Tensor::allocate(
      base::DataType::kDataTypeFp32,
      {config.max_position_embeddings, state.kv_dim},
      base::DeviceType::kDeviceCUDA);
}
...
absl::StatusOr<LlamaForwardResult> CudaLlamaBackend::ForwardToken(
  ...
  for (int32_t layer = 0; layer < config.num_hidden_layers; ++layer) {
    ...
    }
    {
      ...
      EnsureCudaLayerCache(state.layer_caches[layer], config, state);
      kernel::StoreKvCacheCuda(key_tensor, value_tensor,
                               state.layer_caches[layer].cuda_key,
                               state.layer_caches[layer].cuda_value, position,
                               state.kv_dim);
    }
    {
      ...
      kernel::AttentionWithCacheCuda(
          query_tensor, state.layer_caches[layer].cuda_key,
          state.layer_caches[layer].cuda_value, attention_output_tensor,
          position, config.num_attention_heads, state.head_size, state.kv_dim,
          state.kv_mul);
    }
    {
